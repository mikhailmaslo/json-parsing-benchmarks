...
class ApacheTests: XCTestCase {
    ...
    func testCodable() throws {
        measure {
            _ = try! JSONDecoder().decode(ApacheBuilds.self, from: data)
        }
    }

    func testZippy() throws {
        measure {
            _ = try! ZippyJSONDecoder().decode(ApacheBuilds.self, from: data)
        }
    }

    func testExtras() throws {
        measure {
            _ = try! XJSONDecoder().decode(ApacheBuilds.self, from: data)
        }
    }
